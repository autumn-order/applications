<script lang="ts">
    import { ApplicationInterests, ApplicationLanguages, ApplicationRegion, ApplicationReferrer, type ApplicationQuestionsDto, type ApplicationDto } from "$lib/model/application";
    ...
    import { faArrowLeft, faArrowRight, faFloppyDisk, faCheck } from "@fortawesome/free-solid-svg-icons";
    ...
    function removeLanguage(language: Language) {
        if (applicationQuestions.languages.includes(language.value)) {
            applicationQuestions.languages = applicationQuestions.languages.filter(lang => lang !== language.value);
        }
    ...
    function removeInterest(interest: Interest) {
        if (applicationQuestions.interests.includes(interest.value)) {
            applicationQuestions.interests = applicationQuestions.interests.filter(lang => lang !== interest.value);
        }
    ...
    const regions: Region[] = Object.entries(ApplicationRegion).map(([key, value]) => ({ key, value }));
    const referralSources: Referrer[] = Object.entries(ApplicationReferrer).map(([key, value]) => ({ key, value }));

    let languages: Language[] = Object.entries(ApplicationLanguages).map(([key, value]) => ({ key, value }));
    let interests: Interest[] = Object.entries(ApplicationInterests).map(([key, value]) => ({ key, value }));

    let selectedLanguages: Language[] = mapEnumArrayToPairs(ApplicationLanguages, applicationQuestions.languages);
    let selectedInterests: Interest[] = mapEnumArrayToPairs(ApplicationInterests, applicationQuestions.interests);
...
<div class="flex flex-col justify-between flex-grow gap-2 h-full w-full p-2 lg:p-6">
    <form>
        <h1 class="text-gradient text-xl sm:text-2xl font-bold w-full text-center">Step 3. Application Questions</h1>
        <div class="w-full flex flex-wrap">
                ...
                <li class="flex flex-col gap-2">
                    ...
                    <ul class="flex flex-wrap gap-2 {selectedLanguages.length === 0 ? "hidden" : ""}">
                        {#each selectedLanguages as language}
                            <li>
                                <button type="button" on:click={() => removeLanguage(language)} class="border border-color rounded px-2 hover:invert-[0.25]">{language.value}</button>
                            </li>
                        {/each}
                    </ul>
                ...
                <li class="flex flex-col gap-2">
                    ...
                    <ul class="flex flex-wrap gap-2 {selectedInterests.length === 0 ? "hidden" : ""}">
                        {#each selectedInterests as interest}
                            <li>
                                <button type="button" on:click={() => removeInterest(interest)} class="border border-color rounded px-2 hover:invert-[0.25]">{interest.value}</button>
                            </li>
                        {/each}
                    </ul>
            ...
            <ul class="w-full lg:w-1/2 flex flex-col gap-8 px-6 py-4">
                <li class="flex flex-col gap-2">
                    ...
                    <Textarea id="end_goals" name="end_goals" class="w-full p-2 h-28 border rounded" on:input={handleEndGoalInput} bind:value={applicationQuestions.end_goals}/>
                    <p>
                ...
                <li class="flex flex-col gap-2">
                    ...
                    <Textarea id="choose_autumn" name="choose_autumn" class="w-full p-2 h-28 border rounded" on:input={handleWhyAutumnInput} bind:value={applicationQuestions.why_autumn}/>
                    <p>
    ...
    <div class="justify-self-end">
        ...
        <ul>
            {#if questions_complete}
                <li>
                    <p class="text-center">Application questions complete! You can now move on to the next step.</p>
                </li>
            {/if}
        ...
        <ul class="flex w-full justify-between gap-2 mt-6">
            ...
            <li>
                <Button class="flex gap-2 items-center" variant="outline" disabled={(!questions_complete && application !== null)} on:click={() => handleNextStep()}>
                    <p>Next Step</p>
