...
<div class="flex flex-col flex-grow w-full h-full">
	...
	{:else if application && application.status === ApplicationStatus.Rejected}
		{#if application.status === ApplicationStatus.Rejected}
			<AppRejected {application} {user} />
		{/if}
	{:else if application || current_step === ApplicationSteps.Joined}
		<AppProgressIndicator {viewed_step} completed_steps={current_step} {setViewedStep} />
		{#if application}
			<SwitchLocationBanner {user} />
		{:else}
			<MemberBanner />
		{/if}
		{#if viewed_step === ApplicationSteps.SeAT}
			<Step2SetupSeAT {application} {setViewedStep} />
		{:else if viewed_step === ApplicationSteps.Discord}
			<Step3SetupDiscord {application} {setViewedStep} />
		{:else if viewed_step === ApplicationSteps.Questions}
			<Step4AnswerQuestions {application} {setViewedStep} {setCurrentStep} />
		{:else if viewed_step === ApplicationSteps.Review}
			<Step5ApplicationReview {user} {setViewedStep} />
