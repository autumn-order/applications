...
export default class CorporationRepository {
    ...
    async createCorporation(corporation: DbCreateCorporation): Promise<DbCorporation | null> {
        const corp_values: DbCorporation = {
            ...corporation
        }

        const created_corporation = await this.db.insert(schema.Corporation).values(corp_values).returning();
    ...
    async getCorporations(user_ids: number[]): Promise<DbCorporation[]> {
        return await this.db.select().from(schema.Corporation).where(inArray(schema.Corporation.corporation_id, user_ids));
    }

    async getCorporation(corporation_id: number): Promise<DbCorporation | null> {
        const corporation = await this.db.select().from(schema.Corporation).where(eq(schema.Corporation.corporation_id, corporation_id)).limit(1);
