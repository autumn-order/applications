...
export default class SeatRoleUserRepository {
    ...
    private consolidateEntries(entries: { role_id: number, user_id: number }[]): ConsolidatedRoleUsers[] {
    ...
    async get(
        ...
        let query = this.db.select().from(schema.SeatRoleUser)
            .innerJoin(schema.User, eq(schema.SeatRoleUser.seat_user_id, schema.User.seat_user_id));

        if (filters) {
            const conditions = [];

            if (filters.seat_role_ids) {
                conditions.push(inArray(schema.SeatRoleUser.seat_role_id, (filters.seat_role_ids)));
            }
    ...
    async getRoleUserCount(seat_role_id: number): Promise<number> {
        let query = this.db.select({ count: count() }).from(schema.SeatRoleUser).where(eq(schema.SeatRoleUser.seat_role_id, seat_role_id));
    ...
    async create(new_entries: { role_id: number, user_id: number }[]): Promise<DbSeatUserRole[]> {
        const roles = new_entries.map(entry => ({ seat_role_id: entry.role_id, seat_user_id: entry.user_id }));
    ...
    async delete(entries: { role_id: number, user_id: number }[]): Promise<void> {
        ...
        for (const entry of consolidatedEntries) {
            conditions.push(and(eq(schema.SeatRoleUser.seat_role_id, entry.role_id), inArray(schema.SeatRoleUser.seat_user_id, entry.user_ids)));
        }
    ...
    async deleteEntriesByRoleId(role_ids: number[]): Promise<void> {
        await this.db.delete(schema.SeatRoleUser).where(inArray(schema.SeatRoleUser.seat_role_id, role_ids));
    }
}