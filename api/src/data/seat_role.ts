...
export default class SeatRoleRepository {
    ...
    async get(filters?: Partial<DbSeatRoleFilters>, limit?: number, offset?: number): Promise<DbSeatRole[]> {
    ...
    async getRoleMemberCount(role_ids: number[]): Promise<{ seat_role_id: number, count: number }[]> {
        let query = this.db.select({ count: count(), seat_role_id: schema.SeatRoleUser.seat_role_id }).from(schema.SeatRoleUser).groupBy(schema.SeatRoleUser.seat_role_id);
    ...
    async deleteRolesById(role_ids: number[]): Promise<void> {
        await this.db.delete(schema.SeatRole).where(inArray(schema.SeatRole.seat_role_id, role_ids));
    }
}