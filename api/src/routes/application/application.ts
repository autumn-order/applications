import { ApplicationInterests, ApplicationLanguages, ApplicationLocation, ApplicationReferrer, ApplicationRegion } from "model/application"
...
import { getApplication, getApplicationSeatStatus, setApplicationDiscordComplete, setApplicationLocation, setApplicationQuestions } from "service/application"

const putApplicationLocationSchema = z.object({
    location: z.enum(Object.values(ApplicationLocation) as [ApplicationLocation.Highsec, ApplicationLocation.Nullsec]),
})

const putApplicationQuestionsSchema = z.object({
    referrer: z.optional(z.enum(Object.values(ApplicationReferrer) as [
    ...
    region: z.optional(z.enum(Object.values(ApplicationRegion) as [
    ...
    end_goals: z.string(),
    why_autumn: z.string(),
    interests: z.array(z.enum(Object.values(ApplicationInterests) as [
    ...
    languages: z.array(z.enum(Object.values(ApplicationLanguages) as [
...
applicationRoute.get("/", async (ctx: Context): Promise<Response> => {
    ...
    const application = await getApplication(db, ctx, user.user.id);
    if (application instanceof Response) return application;

    return ctx.json(application);
})

applicationRoute.get("/seat-status", async (ctx: Context): Promise<Response> => {
    const db = drizzle(ctx.env.DB);
    ...
    const application = await getApplicationSeatStatus(db, ctx, user.user.id);
    if (application instanceof Response) return application;

    return ctx.json(application);
})

applicationRoute.get("/discord-complete", async (ctx: Context): Promise<Response> => {
    const db = drizzle(ctx.env.DB);
    ...
    const application = await setApplicationDiscordComplete(db, ctx, user.user.id);
    if (application instanceof Response) return application;
...
applicationRoute.put(
    ...
    async (ctx: Context): Promise<Response> => {
        ...
        const application = await setApplicationLocation(db, ctx, user.user.id, body.location);
        if (application instanceof Response) return application;
...
applicationRoute.put(
    ...
    async (ctx: Context): Promise<Response> => {
        ...
        const application = await setApplicationQuestions(db, ctx, user.user.id, body);
        if (application instanceof Response) return application;
