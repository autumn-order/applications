...
settingsRoutes.post(
  ...
  async (ctx: Context) => {
    const settingsRepository = new SettingRepository(ctx.var.db);
...
settingsRoutes.get("/webhooks", async (ctx: Context) => {
  const settingsRepository = new SettingRepository(ctx.var.db);
    ...
    for (const key in body) {
      if (body.hasOwnProperty(key)) {
        ...
        if (value) {
          ...
          const description =
            discordWebhookDescriptions[
              key as keyof typeof discordWebhookDescriptions
            ] || "Unknown Webhook";
          ...
          try {
            ...
            if (error instanceof Error) {
              ctx.status(400);
              return ctx.text(
                `Failed to send test message to ${description} webhook: ${error.message}`,
              );
            } else {
              ctx.status(500);
              return ctx.text(
                `Failed to send test message to ${description} webhook: Unknown error`,
              );
            }
          }
        }
