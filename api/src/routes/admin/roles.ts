...
import { GetSeatRolesDto, GetSeatRoleUsersDto, SeatRoleDto } from "model/seat_role";
...
const postRolePermissionsQuerySchema = z.array(z.enum(Object.values(Permissions) as [
...
roleRoutes.get(
    ...
    async (ctx: Context) => {
        ...
        const member_counts = await seatRoleRepository.getRoleMemberCount(roles.map((role) => role.seat_role_id));

        const role_dtos: SeatRoleDto[] = roles.map(role => ({
            id: role.id,
            seat_role_id: role.seat_role_id,
            name: role.name,
            member_count: member_counts.find(c => c.seat_role_id === role.seat_role_id)?.count ?? 0
        }));
...
roleRoutes.get(
    ...
    async (ctx: Context) => {
        ...
        const role_user_ids = await seatRoleUserRepository.get({ seat_role_ids: [seat_role_id] });
        ...
        const role_user_characters = await getUserCharacters(db, filters, limit, page);
        const role_user_count = await seatRoleUserRepository.getRoleUserCount(seat_role_id);
...
roleRoutes.get(
    ...
    async (ctx: Context) => {
        ...
        const permissions = await getRolePermissions(db, ctx, seat_role_id, page, limit);
...
roleRoutes.post(
    ...
    async (ctx: Context) => {
        ...
        const user = await getUserFromSession(db, ctx, [Permissions.Admin]);
        if (user instanceof Response) return user

        const body: Permissions[] = await ctx.req.json();

        const permissions = await setRolePermissions(db, ctx, seat_role_id, body);
        if (permissions instanceof Response) return permissions;
