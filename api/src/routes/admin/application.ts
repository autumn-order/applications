...
import {
  sendAcceptedApplicationNotification,
...
function parseLocation(
  ...
  if (
    locationParam &&
    Object.values(ApplicationLocation).includes(
      locationParam as ApplicationLocation,
    )
  ) {
...
applicationRoutes.get("/", async (ctx: Context) => {
  ...
  const discordQueryParam: string | undefined =
    ctx.req.query("discord_completed");
  ...
  if (statusQueryParam) {
    const application_status: ApplicationStatus | undefined =
      parseStatus(statusQueryParam);
  ...
  if (locationQueryParam) {
    const application_location: ApplicationLocation | undefined =
  ...
  if (discordQueryParam) {
    const application_discord_completed: boolean | undefined =
      parseBoolean(discordQueryParam);
    if (application_discord_completed === undefined) {
      ctx.status(400);
      return ctx.text("Invalid discord completed parameter");
    } else {
      application_filters.discord_completed = application_discord_completed
        ? 1
        : 0;
    }
  }
  if (seatQueryParam) {
    const application_seat_completed: boolean | undefined =
      parseBoolean(seatQueryParam);
  ...
  const characterRepository = new CharacterRepository(ctx.var.db);
  ...
  if (characterQueryParam) {
    ...
    const result = await characterRepository.get({
      like_character_name: character_name,
    });
  ...
  const applications = await applicationRepository.get(
    application_filters,
    limit,
    offset,
  );
  ...
  for (const application of applications) {
    ...
    if (application.application.reviewer_user_id) {
      const reviewer = await userRepository.getUserMainCharacter(
        application.application.reviewer_user_id,
      );
      if (!reviewer) {
        throw new Error(
          `Reviewing User with ID ${application.application.reviewer_user_id} not found for application ID ${application.application.id}`,
        );
      }
    ...
    applicationDtos.push(
      convertToApplicationDto(
  ...
  const total_application_entries =
    await applicationRepository.getApplicationCount(application_filters);
...
applicationRoutes.put(
  ...
  async (ctx: Context) => {
    ...
    const { status, rejection_reason }: PutApplicationStatusBody =
      await ctx.req.json();
    ...
    if (
      ...
      ctx.status(400);
      return ctx.text(
        "Cannot modify application with status " +
    ...
    if (!result) {
      throw new Error(
        `Failed to update application with ID ${application_id} to status ${status}`,
      );
    }
    ...
    const reviewer_user_dto = convertUserToDto(
      user.user.id,
      user.character,
      user.corporation,
    );
    ...
    if (status === ApplicationStatus.Accepted) {
      await sendAcceptedApplicationNotification(
        ctx.var.db,
        application,
        reviewer_user_dto,
      );
    } else if (status === ApplicationStatus.Rejected) {
      await sendRejectedApplicationNotification(
        ctx.var.db,
        application,
        reviewer_user_dto,
      );
