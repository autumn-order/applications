...
userRoutes.get(
  ...
  async (ctx: Context) => {
    ...
    const user_characters = await getUserCharacters(
      ctx.var.db,
      filters,
      limit,
      page,
    ...
    const total_user_entries =
      await characterRepository.getCharacterCount(filters);
...
userRoutes.get(
  ...
  async (ctx: Context) => {
    ...
    const user_roles_dto = await getUserRoles(ctx, user_id, limit, page);
