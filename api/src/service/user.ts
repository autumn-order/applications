...
export async function getUserCharacters(
  ...
  }[] = await characterRepository.get(filters, limit, offset);

  const user_characters: UserCharactersDto[] = entries
    .reduce((acc, entry) => {
      ...
      if (userDto) {
        ...
        acc.push({
          id: entry.user.id,
          main_character:
            entry.user.main_character_id === entry.character.character_id
              ? characterDto
              : null,
          characters:
            entry.user.main_character_id === entry.character.character_id
    ...
    .filter((user) => user.main_character !== null) as UserCharactersDto[];
