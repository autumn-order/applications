...
async function sendEmbed(
  ...
  try {
    ...
    if (error instanceof Error) {
      console.error(
        `Failed to send new application notification to webhook: ${error.message}`,
      );
    } else {
      console.error(
        `Failed to send new application notification to webhook: Unknown error`,
      );
    }
  }
}

function getCorpInfo(location: string): {
  ...
  if (location === ApplicationLocation.Highsec) {
    return {
      ...
      corporation_logo:
        "https://images.evetech.net/corporations/98784256/logo?size=32",
    };
  } else {
    return {
      corporation_name: "The Order of Autumn",
      corporation_logo:
        "https://images.evetech.net/corporations/98785281/logo?size=32",
    };
  }
}
...
export async function sendNewApplicationNotification(
  ...
  const webhook = await settingsRepository.get(
  ...
  const { corporation_name, corporation_logo } = getCorpInfo(
  ...
  const embed = new MessageBuilder()
    ...
    .setAuthor(
      application.character.character_name,
      `https://images.evetech.net/characters/${application.character.character_id}/portrait?size=32`,
      `https://zkillboard.com/character/${application.character.character_id}/`,
    )
    .setDescription(
      `New application from ${application.character.character_name} to ${corporation_name}.`,
    )
...
export async function sendReviewReadyApplicationNotification(
  ...
  const embed = new MessageBuilder()
    ...
    .setDescription(
      `${application.character.character_name} has completed all application steps to join ${corporation_name} and is ready for review!`,
    )
    .setFooter(corporation_name, corporation_logo)
    .setTimestamp();
...
export async function sendAcceptedApplicationNotification(
  ...
  const corporation_name = getCorpInfo(
    application.application.location,
  ).corporation_name;
  ...
  const embed = new MessageBuilder()
    ...
    .setDescription(
      `${application.character.character_name}'s application to ${corporation_name} has been accepted!`,
    )
    .setFooter(
      `Reviewer: ${reviewer.character_name}`,
      `https://images.evetech.net/characters/${reviewer.character_id}/portrait?size=32`,
    )
    .setTimestamp();
...
export async function sendRejectedApplicationNotification(
  ...
  const embed = new MessageBuilder()
    ...
    .setDescription(
      `${application.character.character_name}'s application to ${corporation_name} has been rejected.`,
    )
...
export async function sendExpiredApplicationNotification(
  ...
  const embed = new MessageBuilder()
    ...
    .setDescription(
      `${application.character.character_name}'s application to ${corporation_name} has expired after 30 days of inactivity.`,
    )
    .setFooter(`${corporation_name}`, corporation_logo)
    .setTimestamp();
...
export async function sendJoinedApplicationNotification(
  ...
  const embed = new MessageBuilder()
    ...
    .setDescription(
      `${application.character.character_name} has joined ${corporation_name}!`,
    )
    .setFooter(`${corporation_name}`, corporation_logo)
    .setTimestamp();
