import CharacterOwnershipRepository, {
...
async function syncSeatUser(
  ...
  const seat_user_id = await fetchSeatCharacterUserId(character_id);

  if (!seat_user_id) {
    return null;
  }

  const existing_seat_user_entry = await userRepository.get(
    { seat_user_id },
    1,
  );
  ...
  if (existing_seat_user_entry[0]) {
    ...
    let ownerships = await characterOwnershipRepository.get(
      { character_id },
      1,
    );
    ...
    if (ownerships[0]) {
      ...
      if (!ownership) {
        throw new Error(
          `Failed to update character ownership for character with id ${character_id} to user with id ${new_user_id}`,
        );
      }
    }
    ...
    if (!existing_user) {
      throw new Error(
        `Failed to retrieve user with id ${new_user_id} after updating character ownership.`,
      );
    }
...
async function getExistingUser(
  ...
  if (!user) {
    throw new Error(
      `User with id ${user_id} not found yet ownership entry is present in violation of foreign key constraint.`,
    );
  }

  if (!user.seat_user_id) {
    const result = await syncSeatUser(db, character_id);
    ...
    if (typeof result === "number") {
      ...
      const seat_user_data = await fetchSeatUser(seat_user_id);

      if (user.main_character_id !== seat_user_data.main_character_id) {
        await createEveCharacter(db, seat_user_data.main_character_id);
...
async function createNewUser(
  ...
  if (typeof result === "number") {
    ...
    await createEveCharacter(db, character_id, character_name);
    ...
    if (!ownership) {
      throw new Error(
        `Failed to create ownership entry for user with id ${existing_user.id} and character with id ${character_id}`,
      );
    }

    if (!existing_user.seat_user_id) {
      throw new Error(
        `User with id ${existing_user.id} has no seat_user_id after sync.`,
      );
    }

    const seat_user_data = await fetchSeatUser(existing_user.seat_user_id);
  ...
  await createEveCharacter(db, character_id);
  ...
  if (!user) {
    throw new Error(
      `Failed to create user with main character id ${character_id}`,
    );
  }
  ...
  if (!ownership) {
    throw new Error(
      `Failed to create ownership entry for user with id ${user.id} and character with id ${character_id}`,
    );
  }
...
export async function getOrCreateUser(
  ...
  const ownership = await characterOwnershipRepository.get(
  ...
  if (ownership[0]) {
    user = await getExistingUser(db, ownership[0].user_id, character_id);
  } else {
    user = await createNewUser(db, owner_hash, character_id, character_name);
  }
...
> {
  ...
  const admin_character_id = Number(process.env.APPLICATION_ADMIN_CHARACTER_ID);

  if (isNaN(admin_character_id)) {
    throw new Error(
      "APPLICATION_ADMIN_CHARACTER_ID is not set or is not a valid number",
    );
  }
  ...
  if (user.character.character_id == admin_character_id) {
    ...
    const user_permissions = await userRepository.getUserPermissions(
      user.user.id,
    );

    const admin = user_permissions.find(
      (entry) => entry.seat_role_permissions.permission === Permissions.Admin,
    );
    ...
    if (admin) {
      ...
      for (const entry of user_permissions) {
        ...
        const permission_enum = Object.values(Permissions).find(
